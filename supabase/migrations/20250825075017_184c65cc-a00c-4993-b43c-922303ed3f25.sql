...
CREATE POLICY IF NOT EXISTS "Students can update their own record"
ON public.students
FOR UPDATE
USING (auth.uid() = user_id);
...
CREATE POLICY IF NOT EXISTS "Teachers can view their own record"
ON public.teachers
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Teachers can insert their own record"
ON public.teachers
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Teachers can update their own record"
ON public.teachers
FOR UPDATE
USING (auth.uid() = user_id);
